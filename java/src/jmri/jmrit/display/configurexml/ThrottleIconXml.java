 ...
 */
public class ThrottleIconXml extends PositionableLabelXml {

    public ThrottleIconXml() {
    }
    ...
    public Element store(Object o) {
        ...
        ThrottleIcon p = (ThrottleIcon) o;

        Element element = new Element("throttleicon");
        
        element.addContent(p.getThrottleFrame().getXml());
...
//        element.setAttribute("globalVariable", p.getNamedGlobalVariable().getName());
        ...
        element.setAttribute("class", "jmri.jmrit.display.configurexml.ThrottleIconXml");
        return element;
    }

    /**
     * Load, starting with the ThrottleIcon element, then all the value-icon
     * pairs
     *
...
*/
        ThrottleIcon l = new ThrottleIcon(p);

        l.getThrottleFrame().setXml(element.getChild("ThrottleFrame"));
